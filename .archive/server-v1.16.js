...
io.on('connection', (socket) => {
    ...
    socket.on('createSession', ({ controllerPassword, presenterPassword, deadline }, callback) => {
        ...
        if (controllerPassword === presenterPassword) {
            return callback({ success: false, message: 'Senhas do Controller e Presenter devem ser diferentes.' });
        }

        // Validação de prazo
        if (deadline && deadline < Date.now()) {
            return callback({ success: false, message: 'O prazo não pode ser no passado.' });
        }
